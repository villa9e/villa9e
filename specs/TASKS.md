...
- [x] Effects: open source filters, zoom, face tracker, object tracker — new "Effects" tab in `/village/create/edit` (CSS filters already existed). Added manual zoom/pan keyframes (`zoomKeyframes` on `useCreateStore`, reuses `TextKeyframe`/`interpolateKeyframes`) applied as a CSS transform on the preview video. Added `lib/create/tracking.ts` using `@mediapipe/tasks-vision` (FaceDetector + ObjectDetector, loaded from jsdelivr/Google CDN, no API key) to auto-generate zoom/pan keyframes that follow a detected face or object through the clip ("Track Face" / "Track Object" buttons). `zoomKeyframes` persisted through upload/publish edit_state.
