...
export async function GET() {
  try {
    ...
    const { data: existing, error: fetchErr } = await admin.from('bank_accounts').select('*').eq('user_id', user.id);
    if (fetchErr) return NextResponse.json({ accounts: [] }); // Table may not exist yet
