...
import {
  MOCK_USER_GOVERNANCE,
  MOCK_STATS,
  MOCK_SUPPLY,
  TIER_BREAKDOWN,
} from '@/lib/vico/mockData';
...
export default function ViCoOverviewPage() {
  ...
  const STATS = [
    ...
    { label: 'Treasury',         value: `$${(s.treasury_usd/1e6).toFixed(1)}M`,       icon: 'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z' },
  ];

  const TIER_COLOR: Record<string,string> = {
    settler: '#B8B4D8', pioneer: '#7F77DD', builder: '#534AB7', elder: '#26215C',
  };
  const tierColor = TIER_COLOR[u.tier] ?? '#534AB7';
      ...
      <div style={{ background: heroBg, padding: '52px 20px 24px' }}>
        ...
        <div style={{ marginBottom: 4 }}>
          ...
          <div style={{ color: 'rgba(196,192,255,0.8)', fontSize: 13, marginTop: 2 }}>
            {(supply.circulating / 1e6).toFixed(1)}M circulating · {(supply.burned / 1e6).toFixed(1)}M burned
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: '20px 16px 0' }}>
        ...
        <div style={{ background: cardBg, border: cardBorder, borderRadius: 12, padding: '16px', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {[
              { icon: 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z', label: 'Min stake to vote', value: '2,000 $VICO' },
              { icon: 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z', label: 'Min stake to propose', value: '10,000 $VICO' },
              { icon: 'M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z', label: 'Discussion period', value: '3 days' },
              { icon: 'M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2', label: 'Voting period', value: '7 days' },
              { icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', label: 'Passing threshold', value: '60% For' },
              { icon: 'M13 10V3L4 14h7v7l9-11h-7z', label: 'Max voting power', value: '5% of circulating supply' },
            ].map(row => (
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            ...
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: isNight ? '#C4C0FF' : '#3C3489', marginBottom: 4 }}>
                ViCo Governance launches in Year 2
              </div>
              <div style={{ fontSize: 12, color: isNight ? '#9B96C8' : '#5C58A8', lineHeight: 1.5 }}>
                All systems are live and tracking — your stake and participation are being recorded. Every vote and proposal you submit now counts toward your governance history.
              </div>
            </div>
          </div>
        </div>
