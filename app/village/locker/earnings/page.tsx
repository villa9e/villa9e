...
type CategoryEarning = { name: string; shared: boolean; monthly: number };
...
export default function Earnings() {
  ...
  const [categories, setCategories] = useState<CategoryEarning[]>([]);

  useEffect(() => {
    fetch('/api/locker/earnings').then(r => r.json()).then(d => {
      setLifetimeTotal(d.lifetimeTotal ?? 0);
      setThisMonthTotal(d.thisMonthTotal ?? 0);
      setEstimatedMonthly(d.estimatedMonthly ?? 0);
      setMonthly(d.monthly ?? []);
      setCategories(d.categories ?? []);
      setPayout(d.payoutPreference === 'vico' ? 'vico' : 'usd');
      setLoading(false);
    }).catch(() => setLoading(false));
  }, []);

  function setPayoutPref(pref: 'usd'|'vico') {
    setPayout(pref);
    fetch('/api/locker/preferences', {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ payout_preference: pref }),
    }).catch(() => {});
  }
  ...
  const pts = monthly.map((m, i) => {
    const x = (i / Math.max(1, monthly.length - 1)) * 280 + 20;
    const y = chartH - (m.amount / maxAmount) * (chartH - 20) - 10;
    return `${x},${y}`;
  });
  const trendPath = 'M' + pts.join(' L');
  const vicoBonusValue = (thisMonthTotal * 1.15);
    ...
    <div style={{ background:c.bg, minHeight:'100vh', maxWidth:480, margin:'0 auto', paddingBottom:88 }}>
      ...
      <div style={{ padding:'16px 16px 0' }}>
        ...
        <div style={{ background:'#412402', borderRadius:20, padding:22, marginBottom:16, position:'relative', overflow:'hidden' }}>
          ...
          <p style={{ color:'#fff', fontSize:28, fontWeight:800, letterSpacing:-1, margin:'0 0 6px' }}>${lifetimeTotal.toFixed(2)}</p>
          ...
          <div style={{ display:'flex', gap:20 }}>
            <div>
              ...
              <p style={{ color:'#FAC775', fontSize:18, fontWeight:800, margin:0 }}>${thisMonthTotal.toFixed(2)}</p>
            </div>
        ...
        <div style={{ background:c.card, border:`1px solid ${c.border}`, borderRadius:16, padding:16, marginBottom:16, overflowX:'auto' }}>
          ...
          {monthly.length === 0 ? (
            <div style={{ textAlign:'center', color:c.textTer, fontSize:12, padding:'20px 0' }}>No earnings recorded yet.</div>
          ) : (
          <svg width="320" height={chartH + 30} viewBox={`0 0 320 ${chartH + 30}`} style={{ display:'block', margin:'0 auto' }}>
            {monthly.map((m, i) => {
              const barW = 16;
              const x = (i / Math.max(1, monthly.length - 1)) * 280 + 20 - barW / 2;
              const barH = (m.amount / maxAmount) * (chartH - 20);
              const y = chartH - barH - 10;
              return (
                <g key={m.month}>
                  <rect x={x} y={y} width={barW} height={Math.max(barH, 0)} rx={3} fill="#EF9F27" opacity={0.85}/>
        ...
        <div style={{ background:c.card, border:`1px solid ${c.border}`, borderRadius:16, marginBottom:16, overflow:'hidden' }}>
          <div style={{ padding:'14px 16px', borderBottom:`1px solid ${c.border}` }}>
            <p style={{ fontWeight:700, fontSize:14, color:c.text, margin:0 }}>Category Breakdown — Estimated Monthly</p>
          </div>
          {categories.map((cat, i) => (
            <div key={cat.name} style={{ padding:'11px 16px', borderBottom:i<categories.length-1?`1px solid ${c.border}`:'none', display:'flex', alignItems:'center', gap:12 }}>
              <div style={{ flex:1 }}>
                <p style={{ fontSize:13, fontWeight:cat.shared?700:500, color:c.text, margin:0 }}>{cat.name}</p>
              </div>
              <span style={{
        ...
        <div style={{ background:c.card, border:`1px solid ${c.border}`, borderRadius:16, padding:16, marginBottom:16 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={() => setPayoutPref('usd')} style={{ flex:1, padding:'10px 0', borderRadius:10, border:`1px solid ${payout==='usd'?'#1D9E75':c.border}`, background:payout==='usd'?'#04342C':'transparent', color:payout==='usd'?'#9FE1CB':c.textSec, fontSize:13, fontWeight:700, cursor:'pointer' }}>
              USD to Village Bank
            </button>
            <button onClick={() => setPayoutPref('vico')} style={{ flex:1, padding:'10px 0', borderRadius:10, border:`1px solid ${payout==='vico'?'#EF9F27':c.border}`, background:payout==='vico'?'#412402':'transparent', color:payout==='vico'?'#FAC775':c.textSec, fontSize:13, fontWeight:700, cursor:'pointer' }}>
              $VICO +15% bonus
            </button>
          </div>
          {payout === 'vico' && (
            <p style={{ fontSize:12, color:'#FAC775', margin:'10px 0 0', background:'#412402', border:'1px solid #EF9F27', borderRadius:8, padding:'8px 12px' }}>
              +15% bonus applied. This month: ${thisMonthTotal.toFixed(2)} USD = ${vicoBonusValue.toFixed(2)} in $VICO value.
            </p>
          )}
        </div>
        ...
        <div style={{ background:c.card, border:`1px solid ${c.border}`, borderRadius:16, padding:16, marginBottom:8 }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:12, marginTop:4, paddingTop:10, borderTop:`1px solid ${c.border}` }}>
            ...
            <div style={{ flex:1, height:8, background:'#04342C', borderRadius:4 }}>
              <div style={{ width:`${Math.min((lifetimeTotal/250)*100, 100)}%`, height:'100%', background:'#1D9E75', borderRadius:4 }}/>
            </div>
            <span style={{ fontSize:12, fontWeight:700, color:'#1D9E75', minWidth:50, textAlign:'right' }}>${lifetimeTotal.toFixed(2)}</span>
            <span style={{ fontSize:10, color:'#9FE1CB' }}>to you</span>
          </div>
          <p style={{ fontSize:11, color:c.textSec, margin:'10px 0 0', lineHeight:1.6 }}>
            Facebook earns ~$164/year from your data and pays you $0. Village has paid you ${lifetimeTotal.toFixed(2)} so far
            {categories.filter(cc => cc.shared).length > 0
              ? ` — you're sharing ${categories.filter(cc => cc.shared).length} ${categories.filter(cc => cc.shared).length === 1 ? 'category' : 'categories'}, estimated at $${estimatedMonthly.toFixed(2)}/month.`
              : '. Share a category in Permissions to start earning.'}
          </p>
        </div>
      </div>
    </div>
