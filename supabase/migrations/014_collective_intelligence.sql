-- Spirit Collective Intelligence — anonymized village-wide patterns
CREATE TABLE IF NOT EXISTS spirit_collective (
  id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  insight     TEXT NOT NULL,
  archetype   TEXT,               -- null = applies to all archetypes
  category    TEXT,               -- goals | habits | community | spirit | general
  metric      TEXT,               -- what was measured (completion_rate, streak_days, etc.)
...
CREATE POLICY "public read collective" ON spirit_collective FOR SELECT USING (true);

-- Seed with initial collective wisdom (grows via cron)
...
-- Cron-updated aggregate function (called by weekly spirit cron)
CREATE OR REPLACE FUNCTION refresh_spirit_collective()
RETURNS void AS $$
BEGIN
  -- Update completion rates by archetype
